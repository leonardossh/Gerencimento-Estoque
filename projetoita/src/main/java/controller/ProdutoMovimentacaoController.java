...
public class ProdutoMovimentacaoController {
	...
	public void movimentarForm(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		form.addRawHtml("<label>Tipo de Movimentação:</label><br>" + "<select name='tipoMovimentacao'>"
				+ "<option value='entrada'>Entrada</option>" + "<option value='saida'>Saida</option>"
				+ "</select><br><br>");

		form.addInput("Tipo (ex: Compra, Venda)", "tipo", "text");
	...
	public void movimentarProduto(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if ("entrada".equalsIgnoreCase(tipoSelecionado)) {
			...
			tipoEnum = "Entrada";
		} else if ("saida".equalsIgnoreCase(tipoSelecionado)) {
